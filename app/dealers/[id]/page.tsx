    ...
    <main style={{ minHeight: "100dvh", background: "#F8F8F8", paddingBottom: 40 }}>
      ...
      <div style={{ padding: "16px" }}>
        <div style={{ background: WHITE, border: "0.5px solid #E8E8E8", borderRadius: 2, padding: "16px", marginBottom: 12, display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 300, color: BLACK, marginBottom: 4 }}>
              ¥{dealer.hourly_rate.toLocaleString()}<span style={{ fontSize: 12, color: GRAY3, fontWeight: 400 }}> / hour</span>
            </div>
            <span style={{ display: "inline-block", fontSize: 9, padding: "2px 8px", borderRadius: 1, background: "#F5F5F5", color: "#333", letterSpacing: 2, textTransform: "uppercase" }}>
              {VENUE_LABEL[dealer.venue_type]}
            </span>
            <p style={{ fontSize: 11, color: GRAY3, marginTop: 6 }}>
              経験{dealer.experience_years}年
            </p>
          </div>
        </div>

        <div style={{ background: WHITE, border: "0.5px solid #E8E8E8", borderRadius: 2, padding: "14px 16px", marginBottom: 12 }}>
          <div style={{ fontSize: 9, color: GRAY3, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 8 }}>About</div>
        ...
        <div style={{ background: WHITE, border: "0.5px solid #E8E8E8", borderRadius: 2, overflow: "hidden", marginBottom: 12 }}>
          <div style={{ fontSize: 9, color: GRAY3, letterSpacing: ".1em", textTransform: "uppercase", padding: "14px 16px 8px" }}>Specs</div>
          {[
            { label: "Games",  value: dealer.game_types.join(" · ") },
            { label: "Areas",  value: dealer.areas.join(" · ") },
            { label: "Venue",  value: VENUE_LABEL[dealer.venue_type] },
          ].map(({ label, value }) => (
        ...
        {dealer.tags && dealer.tags.length > 0 && (
          <div style={{ background: WHITE, border: "0.5px solid #E8E8E8", borderRadius: 2, padding: "14px 16px", marginBottom: 20 }}>
            <div style={{ fontSize: 9, color: GRAY3, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 10 }}>Tags</div>
            {dealer.tags.map((tag) => (
              <span key={tag} style={{ display: "inline-block", fontSize: 11, padding: "3px 10px", borderRadius: 1, background: "#F5F5F5", color: "#333", border: "0.5px solid #E8E8E8", marginRight: 6, marginTop: 6, letterSpacing: 1 }}>
                {tag}
              </span>
            ))}
          </div>
        ...
        <Link href={`/dealers/${dealer.id}/request`} style={{ display: "block", padding: 16, background: BLACK, color: WHITE, borderRadius: 2, fontSize: 12, fontWeight: 400, textAlign: "center", textDecoration: "none", letterSpacing: 3, textTransform: "uppercase" }}>
          Book This Dealer
        </Link>
      </div>
    </main>
