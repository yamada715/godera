...
import { supabase, VENUE_LABEL, type DealerRow } from "@/lib/supabase";
...
export default function RequestPage() {
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from("requests").insert({
      ...
      location, email, note, hours, total_fee: totalFee,
      status: "pending",
    });
    setSending(false);
    ...
    return (
      <main style={{ minHeight: "100dvh", background: "#F8F8F8", display: "flex", alignItems: "center", justifyContent: "center", padding: 32 }}>
        <div style={{ textAlign: "center", maxWidth: 280 }}>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 400, color: BLACK, marginBottom: 8, letterSpacing: 1 }}>依頼を送信しました</h2>
          ...
          <Link href="/dealers" style={{ display: "block", padding: 14, background: BLACK, color: WHITE, borderRadius: 2, fontSize: 11, fontWeight: 400, textAlign: "center", textDecoration: "none", letterSpacing: 3, textTransform: "uppercase" }}>
            Back to Dealers
          </Link>
        </div>
      </main>
  ...
  const labelStyle: React.CSSProperties = {
    fontSize: 11, color: GRAY3, fontWeight: 400, marginBottom: 5,
  ...
  return (
    <main style={{ minHeight: "100dvh", background: "#F8F8F8", paddingBottom: 40 }}>
      <header style={{ background: BLACK, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <span style={{ fontSize: 11, color: WHITE, letterSpacing: 3, textTransform: "uppercase" }}>Book</span>
        <div style={{ width: 32 }} />
      </header>
      ...
      <div style={{ padding: "16px" }}>
        <div style={{ background: WHITE, border: "0.5px solid #E8E8E8", borderRadius: 2, padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          <div style={{ width: 44, height: 44, borderRadius: 2, background: "#F0F0F0", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            {dealer.photo_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={dealer.photo_url} alt={dealer.name} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 2 }} />
            ) : (
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <circle cx="12" cy="9" r="5" fill="#CCCCCC" />
                <ellipse cx="12" cy="20" rx="9" ry="5" fill="#CCCCCC" />
              </svg>
            )}
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 500, color: BLACK }}>{dealer.name}</div>
            <div style={{ fontSize: 13, color: GRAY3 }}>¥{dealer.hourly_rate.toLocaleString()} / hour</div>
          </div>
        </div>

        <div style={{ background: WHITE, border: "0.5px solid #E8E8E8", borderRadius: 2, padding: "16px", marginBottom: 12 }}>
          <div style={{ fontSize: 10, color: GRAY3, letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>依頼内容</div>
        ...
        <div style={{ background: hours > 0 ? "#F5F5F5" : "#F8F8F8", border: "0.5px solid #E8E8E8", borderRadius: 2, padding: "14px 16px", marginBottom: 20 }}>
          <div style={{ fontSize: 10, color: GRAY3, letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>料金目安</div>
          {hours > 0 ? (
            <>
        ...
        <button onClick={handleSubmit} disabled={sending} style={{ width: "100%", padding: 15, background: sending ? "#999" : BLACK, color: WHITE, border: "none", borderRadius: 2, fontSize: 12, fontWeight: 400, cursor: sending ? "not-allowed" : "pointer", letterSpacing: 3, textTransform: "uppercase", marginBottom: 10 }}>
          {sending ? "Sending..." : "Send Request"}
        </button>
        <Link href={`/dealers/${dealer.id}`} style={{ display: "block", padding: 12, background: "transparent", color: GRAY3, border: "0.5px solid #E8E8E8", borderRadius: 2, fontSize: 12, textAlign: "center", textDecoration: "none", letterSpacing: 2, textTransform: "uppercase" }}>
          Cancel
        </Link>
      </div>
    </main>
